...
from flask.ext.login import LoginManager, login_required, login_user, logout_user, login_required
import base64
    ...
    def get(self):
        ...
        if 'username' in session and session['username'] is not '':
            ...
            if user:
                ...
                return {"meta": buildMeta(), "error": "No Session Found for '" + session['username'] + "'",
                        "data": None}, 401
        else:
...
class ApiMe(Resource):
    ...
    def put(self, user_id=None):
        ...
        if user is not None:
            if request_is_json():
                ...
                return {"meta": buildMeta(), "error": "Unable to process, no content type was provided",
                        "data": None}, 202
            else:
                return {"meta": buildMeta(), "error": "Unable to process " + str(request.content_type),
                        "data": None}, 202
    ...
    def post(self, user_id=None):
        ...
        if new_password != confirm_new_password:
            application.logger.debug(
                "User '" + email + "' Registration failed, new_password does not match confirm_new_password")
        ...
        if pay_recurrance_flag is None or next_pay_date is None:
            ...
            application.logger.debug(
                "User '" + email + "' Registration failed, Pay Recurrance and Next Pay Date is Required")
        ...
        newUser = User(username=email, password=new_password, email=email, first_name=first_name, last_name=last_name,
                       next_pay_date=next_pay_date, pay_recurrance_flag=pay_recurrance_flag,
                       account_balance_amount=account_balance_amount, confirm_token=confirm_token)
    ...
    def delete(self, user_id=None):
        return {"meta": buildMeta(),
                "data": "Tisk, tisk. You cannot just simply 'DELETE' an account! there are protocols!",
                "error": None}, 200
...
class ApiBill(Resource):
    ...
    def get(self, bill_id=None):
        ...
        if billId is not None:
            ...
            return {"meta": buildMeta(), "data": [i.serialize for i in Bill.query.filter_by(**newDict).all()],
                    "error": None}
    ...
    def post(self, bill_id=None):
        ...
        newBill = Bill(user_id=user.id, name=name, description=description, due_date=due_date,
                       ...
                       payment_type_ind=payment_type_ind, check_number=check_number,
                       payment_processing_flag=payment_processing_flag)
...
class ApiPaymentPlan(Resource):
            ...
            if accepted_flag is not None:
                ...
                # Get existing payment plans
                    ...
                    if bill_id is not None:
                        ...
                        payment_plans = [i.serialize for i in
                                         Payment_Plan.query.filter(Payment_Plan.id.in_(idList)).all()]

                    else:
                        payment_plans = [i.serialize for i in
                                         Payment_Plan.query.filter_by(user_id=user.id, accepted_flag=accepted_flag)]
            else:
    ...
    def put(self, payment_plan_id=None):
        ...
        if payment_plan_items is not None:
            ...
            for payment_plan_item in payment_plan_items:
                new_payment_plan_items.append(
                    Payment_Plan_Item(payment_plan_id=payment_plan_id, user_id=payment_plan_item['user_id'],
                                      bill_id=payment_plan_item['bill_id'], amount=payment_plan_item['amount']))
        ...
        if accepted_flag is not None:
            ...
            if accepted_flag is True:
                ...
                for payment_plan_item in payment_plan.payment_plan_items:
                    ...
                    total_paid = db_session.query(func.sum(Payment_Plan_Item.amount).label('sum_amount')).filter(
                        Payment_Plan_Item.bill_id == payment_plan_item.bill_id).filter(
                        Payment_Plan_Item.accepted_flag == True).first()
    ...
    def delete(self, payment_plan_id=None):
        ...
        if bill is not None:
            ...
            return {"meta": buildMeta(), "error": "Payment Plan #" + payment_plan_id + " Could not be found",
                    "data": None}


api.add_resource(ApiPaymentPlan, '/api/payment_plan', '/api/payment_plan/',
                 '/api/payment_plan/<string:payment_plan_id>')
        ...
        if payment_plan_item_id is None:
            if bill_id is not None:
                ...
                if bill is not None:
                    ...
                    return {"meta": buildMeta(), "error": "Bill #" + str(bill_id) + " Could not be found",
                            "data": None}, 202
            else:
            ...
            if payment_plan_item is not None:
                ...
                return {"meta": buildMeta(),
                        "error": "Payment Plan Item #" + str(payment_plan_item_id) + " Could not be found",
                        "data": None}


api.add_resource(ApiPaymentPlanItem, '/api/payment_plan_item', '/api/payment_plan_item/',
                 '/api/payment_plan_item/<string:payment_plan_item_id>')
    ...
    def post(self, feedback_id=None):
        ...
        if rating is not None and feedback is not None:
            ...
            send_email('New Feedback',
                       ['"Robert Donovan" <admin@mixfin.com>', '"David Larrimore" <david.larrimore@mixfin.com'], None,
                       html_message)
...
class ApiConfirmEmail(Resource):
    ...
    def delete(self, user_id=None):
        return {"meta": buildMeta(),
                "data": "Tisk, tisk. You cannot just simply 'DELETE' a confirmaion email! there are protocols!",
                "error": None}, 200
...
class ApiPasswordRecovery(Resource):
    ...
    def put(self, email_address=None):
        ...
        if password_recovery_token is not None and password_recovery_token == user.confirm_token:
            if new_password and confirm_new_password:
                ...
                return {"meta": buildMeta(),
                        "error": "When changing passwords, both password and confirmation are required."}, 201
            else:
    ...
    def delete(self, user_id=None):
        return {"meta": buildMeta(),
                "data": "Tisk, tisk. You cannot just simply 'DELETE' a password confirmation! there are protocols!",
                "error": None}, 200


api.add_resource(ApiPasswordRecovery, '/api/recover_password', '/api/recover_password/',
                 '/api/recover_password/<string:email_address>')
...
def send_email_confirmation_email(first_name, last_name, email, confirm_token):
    ...
    send_email('2Weeks Email Confirmation', ['"' + first_name + ' ' + last_name + '" <' + email + '>',
                                             '"David Larrimore" <davidlarrimore@gmail.com>'], html_message,
               html_message)
...
def send_password_recovery_email(user):
    ...
    send_email('2Weeks Password Recovery', ['"' + user.first_name + ' ' + user.last_name + '" <' + user.email + '>',
                                            '"David Larrimore" <davidlarrimore@gmail.com>'], html_message, html_message)
